...
namespace NS_NaviCommon
{
  ...
  void Communicator::receiveProcess()
  {
    ...
    while(running)
    {
      ...
      if(data->type == COMMUNICATION_DATA_TYPE_NULL || data->reason == COMMUNICATION_DATA_REASON_NULL)
      {
        continue;
      }
  ...
  Communicator::sendAndWait(CommData** request)
  {
    ...
    holder_cond_lock.lock();
    while((findRequest((*request)->sequence, (*request)->reason) == *request) && ((response = findResponse(
        (*request)->sequence, (*request)->reason)) == NULL))
    {
      holder_cond.timed_wait(
          holder_cond_lock,
          boost::get_system_time() + boost::posix_time::seconds(
              HOLDER_COND_TIMEOUT));
    }

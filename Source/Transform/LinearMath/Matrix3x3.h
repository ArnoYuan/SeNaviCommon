...
#define Matrix3x3Data	Matrix3x3DoubleData 
  ...
  public:
    ...
    void getRotation(Quaternion& q) const
    {
      ...
      {
        int i =
            m_el[0].x() < m_el[1].y() ? (m_el[1].y() < m_el[2].z() ? 2 : 1) : (
    ...
    __attribute__((deprecated)) void getEulerZYX(
        tfScalar& yaw, tfScalar& pitch, tfScalar& roll,
    ...
    void diagonalize(Matrix3x3& rot, tfScalar threshold, int maxSteps)
    {
      ...
      for(int step = maxSteps; step > 0; step--)
      {
        ...
        tfScalar t = threshold * (tfFabs(m_el[0][0]) + tfFabs(m_el[1][1]) + tfFabs(
            m_el[2][2]));
        if(max <= t)
        {
        ...
        {
          t = (theta >= 0) ? 1 / (theta + tfSqrt(1 + theta2)) : 1 / (theta - tfSqrt(
              1 + theta2));
          cos = 1 / tfSqrt(1 + t * t);
          sin = cos * t;
        }
  ...
  TFSIMD_FORCE_INLINE tfScalar Matrix3x3::determinant() const
  {
    return tfTriple((*this)[0], (*this)[1], (*this)[2]);
  }

  TFSIMD_FORCE_INLINE Matrix3x3 Matrix3x3::absolute() const
  {
  ...
  TFSIMD_FORCE_INLINE Matrix3x3 Matrix3x3::adjoint() const
  {
  ...
  TFSIMD_FORCE_INLINE Matrix3x3 Matrix3x3::inverse() const
  {
  ...
  TFSIMD_FORCE_INLINE Matrix3x3 Matrix3x3::transposeTimes(
  ...
  TFSIMD_FORCE_INLINE Matrix3x3 Matrix3x3::timesTranspose(
      const Matrix3x3& m) const
  {
...
///for serialization
  ...
  TFSIMD_FORCE_INLINE void Matrix3x3::serialize(
  ...
  TFSIMD_FORCE_INLINE void Matrix3x3::serializeFloat(
  ...
  TFSIMD_FORCE_INLINE void Matrix3x3::deSerialize(
  ...
  TFSIMD_FORCE_INLINE void Matrix3x3::deSerializeFloat(
  ...
  TFSIMD_FORCE_INLINE void Matrix3x3::deSerializeDouble(
